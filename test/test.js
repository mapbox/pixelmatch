...
const readImage = name => new Promise((res, rej) => {
    const png = new PNG();
    fs.createReadStream(path.join(__dirname, '/fixtures/' + name + '.png'))
        .pipe(png)
        .on('parsed', data => res(png, data))
        .on('error', rej);
});
