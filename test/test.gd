...
func test_diffs():
	if not equality_asserter:
		equality_asserter = FuncRef.new()
		equality_asserter.set_instance(self)
		equality_asserter.function = "assert_eq"
	
	diff_test("icon", "icon", "icon_emptydiff", options, 0)
	diff_test("icon", "icon_alt", "icon_diff", options, 1635)
...
func diff_test(img1_name, img2_name, diff_name, options: Dictionary, expected_mismatch):
	...
	if UPDATE:
		...
		equality_asserter.call_func(diff_diff, 0)

	equality_asserter.call_func(mismatch, expected_mismatch, 'number of mismatched pixels %s %s' % [img1_name, img2_name])
	equality_asserter.call_func(mismatch, mismatch2, 'number of mismatched pixels without diff %s %s' % [img1_name, img2_name])
